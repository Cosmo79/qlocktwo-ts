...
import letters_ro from './letters_ro';

const appElement = document.getElementById('app_ro');
...
const clockElement = createClockElement(letters_ro);
appElement?.appendChild(clockElement);

const flushClockElement = (hours: number, minutes: number) => {
    const [h, m] = [hours % 12 || 12, minutes];
    const amOrPm = hours < 12 ? 'am' : 'pm';
    clockElement.className = `clock este ora h-${h} m-${m} ${amOrPm}`;
};

const flushClockElementWithDate = (date: Date) => flushClockElement(date.getHours(), date.getMinutes());
...
const initWithHoursAndMinutes = (hours: number, minutes: number, running?: boolean) => {
    flushClockElement(hours, minutes);
    let runningMinutes = hours * 60 + minutes;
    if (running) {
        setInterval(() => {
            runningMinutes = (runningMinutes + 1) % (24 * 60);
            flushClockElement(Math.floor(runningMinutes / 60), runningMinutes % 60);
        }, 1000);
    }
};
